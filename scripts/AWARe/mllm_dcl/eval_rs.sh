...
QUESTION_FILE="datasets/RS/test.json"
...
for IDX in $(seq 0 $((CHUNKS-1))); do
    CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python -m llava.eval.CoIN.model_ai2d \
        ...
        --answers-file "$RESULT_DIR/RS/${CHUNKS}_${IDX}.jsonl" \
        --num-chunks "$CHUNKS" \
        --chunk-idx "$IDX" \
        --temperature 0 \
        --conv-mode vicuna_v1 &
...
output_file="$RESULT_DIR/RS/merge.jsonl"
> "$output_file"

for IDX in $(seq 0 $((CHUNKS-1))); do
    cat "$RESULT_DIR/RS/${CHUNKS}_${IDX}.jsonl" >> "$output_file"
done
